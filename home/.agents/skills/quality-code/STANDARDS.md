...
Use the project's established result type. In an Effect codebase, use Effect's result/error facilities. If the project has no result abstraction, use a small local tagged union:
...
Expected failures should use custom tagged errors, generally extending `Error`, or `Schema.TaggedErrorClass` in Effect codebases.
...
- common `Result` helpers when the project has no established result abstraction
...
Prefer expressive names and types over comments. Add a comment or JSDoc only when callers need information the code cannot express, such as a non-obvious invariant, compatibility constraint, trade-off, or safety justification. Export visibility alone is not a reason to add documentation. Omit comments and JSDoc from self-explanatory functions, methods, types, and fields.
...
- Share types end-to-end from schemas, database definitions, server contracts, and client APIs using the project's established tools.
- Derive shapes instead of restating them with `Pick`, `Omit`, `Parameters`, `ReturnType`, `Awaited`, `typeof`, and indexed access types.
- Derive wrapper types from the APIs they wrap, such as `Parameters<typeof fetch>` or `Pick<typeof globalThis, "fetch">`.
- Avoid interfaces that duplicate existing entities, events, or dependency APIs unless they define a true public boundary.

## Browser, fetch, and body handling

- Clone `Request` and `Response` objects before recorder or test inspection so caller-visible bodies are not consumed.
- When wrapping `fetch`, preserve its behavior and rethrow original errors.
- Keep install, uninstall, and cleanup paths idempotent; do not restore over a newer wrapper.
- Track asynchronous background recording work and expose a flush operation when tests need deterministic assertions.

## Regular expressions and matching

- Regular expressions with `g` or `y` are stateful. Clone or reset them before repeated `.test()` calls.
- Render regular expressions as `/pattern/flags` in assertion failures rather than allowing them to serialize as `{}`.
