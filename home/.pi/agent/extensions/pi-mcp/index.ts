...
const McpProxyParams = Type.Object({
  tool: Type.Optional(
    Type.String({
      description:
        "MCP tool name to call, usually the prefixed <server>_<tool> name from search/list.",
    })
  ),
  args: Type.Optional(
    Type.String({
      description: 'Tool arguments as a JSON object string, for example \'{"key":"value"}\'.',
    })
  ),
  connect: Type.Optional(
    Type.String({
      description: "Connect or reconnect one configured MCP server and refresh its tool metadata.",
    })
  ),
  describe: Type.Optional(
    Type.String({ description: "MCP tool name to describe, including its parameters." })
  ),
  search: Type.Optional(
    Type.String({ description: "Search MCP tools by server, name, and description." })
  ),
  regex: Type.Optional(
    Type.Boolean({
      description:
        "Treat search as a JavaScript regular expression instead of space-separated terms.",
    })
  ),
  server: Type.Optional(
    Type.String({ description: "Filter list/search/describe/call to a specific MCP server." })
  ),
  action: Type.Optional(
    Type.String({ description: "Optional action: 'status', 'resources', or 'read-resource'." })
  ),
  uri: Type.Optional(Type.String({ description: "Resource URI for action 'read-resource'." })),
})

/** Registers the MCP client extension with Pi. */
export default function piMcpExtension(pi: ExtensionAPI) {
  ...
  function startBackgroundConnectionRefresh(
  ...
  function registerDynamicTools() {
    ...
    for (const entry of activeManager.getToolEntries()) {
      ...
      pi.registerTool({
        ...
        description:
          entry.tool.description || `Call MCP tool ${entry.name} on server ${entry.server}`,
        promptSnippet: `Call MCP tool ${entry.name} on server ${entry.server}`,
        promptGuidelines: [
          `Use ${entry.key} only when the user needs the ${entry.name} MCP tool from server ${entry.server}.`,
        ],
  ...
  async function executeMcpProxy(params: unknown, signal: AbortSignal | undefined) {
    ...
    if (action === "read-resource")
      return proxyReadResource(server, optionalString(args, "uri"), signal)
    if (action !== undefined && action !== "status") {
      return proxyText(
        `Unknown MCP action "${action}". Supported actions: status, resources, read-resource.`,
        {
          mode: "error",
  ...
  async function proxySearch(
    ...
    const lines = [
      `Found ${matches.length} MCP tool${matches.length === 1 ? "" : "s"} matching "${query}":`,
      "",
    ]
  ...
  async function proxyList(server: string, signal: AbortSignal | undefined) {
    if (!config.servers[server]) {
      return proxyText(
        `MCP server "${server}" is not configured. Use mcp({}) to see available servers.`,
        {
          mode: "list",
          server,
    ...
    if (entries.length === 0) {
      ...
      return proxyText(
        `MCP server "${server}" has no connected tools (status: ${status}). Use mcp({ connect: "${server}" }) to retry.`,
        {
          mode: "list",
          server,
          count: 0,
  ...
  async function proxyResources(server: string | undefined, signal: AbortSignal | undefined) {
    ...
    if (server && !resourceServers.includes(server)) {
      return proxyText(
        `MCP server "${server}" does not support resources. Available resource servers: ${resourceServers.join(", ") || "none"}.`,
        {
          mode: "resources",
          server,
  ...
  async function proxyReadResource(
    ...
    if (!server)
      return proxyText("read-resource requires `server`.", {
        mode: "read-resource",
        error: "missing_server",
      })
    if (!uri)
      return proxyText("read-resource requires `uri`.", {
        mode: "read-resource",
        server,
        error: "missing_uri",
      })
  ...
  function proxyStatus() {
    ...
    if (servers.length === 0)
      return proxyText("No MCP servers configured.", { mode: "status", servers: [], totalTools: 0 })
    lines.push(
      `MCP proxy: ${servers.length} server${servers.length === 1 ? "" : "s"}, ${entries.length} connected tool${entries.length === 1 ? "" : "s"}`,
      ""
    )
    for (const [name, serverConfig] of servers) {
      ...
      lines.push(
        `${name}: ${status.status}, ${count} tool${count === 1 ? "" : "s"}${target ? ` (${target})` : ""}`
      )
    }
    lines.push(
      "",
      'Use mcp({ search: "..." }) to find tools or mcp({ server: "name" }) to list one server.'
    )
    return proxyText(lines.join("\n"), {
      mode: "status",
      servers: servers.map(([name]) => ({
        ...
        tools: entries.filter((entry) => entry.server === name).length,
      })),
      totalTools: entries.length,
    })
  }

  async function ensureProxyToolMetadata(
    toolName: string,
    server: string | undefined,
  ...
  async function ensureProxySearchMetadata(
    server: string | undefined,
    options: CancellableOptions
  ) {
    if (server) {
    ...
    if (matches.length > 1) {
      return {
        ...
        details: {
          ...
          matches: matches.map((entry) => entry.key),
        },
      }
    }
    return {
  ...
  function registerResourceTools() {
    ...
    pi.registerTool({
      ...
      description:
        "Read a specific resource from an MCP server using the server name and resource URI.",
  ...
  function updateMcpStatus(ctx: ExtensionContext) {
    const count = Object.values(manager?.status() ?? {}).filter(
...
function buildSearchPattern(
  ...
  try {
    ...
    return {
      error: `Invalid MCP search regex: ${query}`,
      details: { mode: "search", error: "invalid_regex", query },
    }
  }
}
