...
export async function showMcpManagerOverlay(
  ...
  return ctx.ui.custom<McpManagerAction>(
    (tui, theme, _keybindings, done) =>
      new McpManagerOverlay(tui, theme, view, selectedServerName, done),
    {
      overlay: true,
      overlayOptions: {
        anchor: "center",
...
class McpManagerOverlay {
  ...
  handleInput(data: string): void {
    ...
    else if (data === "a" && server.authStatus !== undefined) {
      this.done({ _tag: "authenticate", server })
    } else if (data === "l" && server.authStatus !== undefined) {
      this.done({ _tag: "logout", server })
  ...
  render(width: number): string[] {
    ...
    const body =
      innerWidth < 60
        ? this.renderServerList(innerWidth, bodyHeight).map((line) =>
            frameLine(this.theme, line, innerWidth)
          )
    ...
    return [
      ...
      frameLine(
        ...
        this.theme.fg(
          "dim",
          "↑↓ move • enter/c connect • d disconnect • a auth • l logout • p prompt • r reload • esc close"
        ),
        innerWidth
      ),
  ...
  private renderHeader(width: number): string {
    ...
    const connected = this.view.servers.filter(
      (server) => server.status.status === "connected"
    ).length
  ...
  private renderServerList(width: number, height: number): string[] {
    ...
    const start = Math.max(
      ...
      Math.min(
        ...
        Math.max(0, this.view.servers.length - height)
      )
    )
    ...
    for (let index = start; index < end; index += 1) {
      ...
      lines.push(
        selected
          ? this.theme.fg("accent", this.theme.bold(fitLine(row, width)))
          : fitLine(row, width)
      )
    }
  ...
  private renderSelectedDetails(width: number, height: number): string[] {
    ...
    if (server === undefined) {
      return padLines(
        [this.theme.fg("dim", "Add servers to your MCP configuration, then press r to reload.")],
        height
      )
    }
    ...
    if (server.authStatus !== undefined) {
      lines.push(`${this.theme.fg("muted", "OAuth:")} ${server.authStatus}`)
    }
    ...
    if (server.target !== undefined) {
      lines.push("", this.theme.fg("muted", "Target:"), ...wrapPlainText(server.target, width))
    }
    if (this.view.configSource !== undefined) {
      lines.push(
        "",
        this.theme.fg("muted", "Config:"),
        ...wrapPlainText(this.view.configSource, width)
      )
    }
    return padLines(
...
function combineColumns(
  ...
  for (let index = 0; index < rows; index += 1) {
    lines.push(
      `${fitLine(left[index] ?? "", leftWidth)}${separator}${fitLine(right[index] ?? "", rightWidth)}`
    )
  }
  return lines
}
