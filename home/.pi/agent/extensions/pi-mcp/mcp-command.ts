...
import {
  showMcpManagerOverlay,
  type McpManagerView,
  type McpServerView,
} from "./mcp-manager-overlay.js"
import { parseThrownValue } from "./thrown-value.js"
...
export interface McpCommandDependencies {
  ...
  readonly rememberConnection: (server: string, status: McpStatus) => Promise<void>
  readonly forgetConnection: (server: string) => Promise<void>
...
export async function runMcpCommand(
  ...
  let manager = await dependencies.ensureManager(ctx)
  if (ctx.mode !== "tui") {
    ...
    try {
      switch (action._tag) {
        ...
        case "reload":
          ...
          manager = await dependencies.ensureManager(ctx)
        ...
        case "connect": {
          ...
          await dependencies.rememberConnection(action.server.name, status)
        ...
        case "disconnect":
          ...
          await dependencies.forgetConnection(action.server.name)
          dependencies.refreshRuntime(ctx)
          ctx.ui.notify(`Disconnected MCP server ${action.server.name}`, "info")
          continue
        case "authenticate": {
          const status = await runMcpOperation(ctx, "authenticating", () =>
            manager.authenticate(action.server.name, async (url) =>
              dependencies.showAuthorizationUrl(url)
            )
          )
          await dependencies.rememberConnection(action.server.name, status)
          dependencies.refreshRuntime(ctx)
        ...
        case "logout":
          ...
          continue
        case "prompt": {
          const sent = await runMcpPrompt(
            ctx,
            manager,
            action.server.name,
            dependencies.sendUserMessage
          )
          if (sent) return
          continue
        }
      ...
      ctx.ui.notify(mcpCommandError(parseThrownValue(error)), "error")
    }
  }
}

async function createMcpManagerView(
  manager: McpManager,
  config: McpConfig
): Promise<McpManagerView> {
  const statuses = manager.status()
  const clients = manager.connectedClients()
  const toolCounts = new Map<string, number>()
  for (const tool of manager.getToolEntries()) {
  ...
  return {
    ...
    toolMode: config.toolMode ?? "proxy",
...
async function confirmMcpLogout(ctx: ExtensionCommandContext, server: string): Promise<boolean> {
  return ctx.ui.confirm(
    ...
    `Log out of ${server}. You will need to authenticate again.`
  )
}
...
async function runMcpPrompt(
  ...
  const result = await runMcpOperation(ctx, "loading-prompts", () =>
    manager.prompts({ signal: undefined })
  )
...
function notifyConnectionStatus(
  ...
  ctx.ui.notify(
    `MCP server ${server}: ${status.status}`,
    status.status === "needs_auth" ? "warning" : "info"
  )
}
...
function mcpCommandError(error: ReturnType<typeof parseThrownValue>): string {
  return error.kind === "error"
    ? `MCP manager operation failed: ${error.message}`
