...
export class McpManager {
  ...
  async initialize(config: McpConfig, options: McpInitializeOptions): Promise<void> {
    ...
    for (const [name, serverConfig] of Object.entries(this.config.servers)) {
      this.statuses.set(
        name,
        isConfigDisabled(serverConfig) ? { status: "disabled" } : { status: "disconnected" }
      )
    }
  ...
  async resources(
    ...
    if (server) {
      ...
      const resources = await listResources(
        managed.client,
        managed.config.timeout ?? this.config.timeout ?? DEFAULT_TIMEOUT,
        options.signal
      )
      return {
  ...
  async prompts(options: CancellableOptions): Promise<McpPromptsResult> {
    const targets = Array.from(this.clients).filter(
      ([, managed]) => !!managed.client.getServerCapabilities()?.prompts
    )
    const collected = await collectPartial(targets, options.signal, async (name, managed) => {
  ...
  async readResource(clientName: string, uri: string, options: CancellableOptions) {
    const managed = this.clients.get(clientName)
    if (!managed) throw new Error(`MCP server "${clientName}" is not connected`)
    if (!managed.client.getServerCapabilities()?.resources)
      throw new Error(`MCP server "${clientName}" does not support resources`)
  ...
  async authenticate(name: string, onAuthorizationUrl?: (url: string) => void | Promise<void>) {
    ...
    if (!result.authorizationUrl) {
      if (!result.client)
        return {
          status: "failed",
          error: "OAuth did not return a connected client",
        } satisfies McpStatus
      ...
      if (!result.transport)
        return {
          status: "failed",
          error: "OAuth did not return a connected transport",
        } satisfies McpStatus
      ...
      return (
        this.statuses.get(name) ?? {
          status: "failed",
          error: "OAuth did not store a connected status",
        }
      )
    }
  ...
  private async connectLocal(
    name: string,
    serverConfig: Extract<McpServerConfig, { type: "local" }>,
    options: CancellableOptions
  ) {
    const [command, ...args] = serverConfig.command
    if (!command)
      return { status: { status: "failed" as const, error: "Local MCP command is empty" } }
    const cwd = serverConfig.cwd
      ? path.resolve(this.options.cwd, resolveHome(serverConfig.cwd))
      : this.options.cwd
  ...
  private async connectRemote(
    ...
    if (!url)
      return { status: { status: "failed" as const, error: `Invalid MCP URL for "${name}"` } }
    ...
    const transports: Array<{ name: string; transport: TransportWithAuth }> = [
      {
        ...
        transport: new StreamableHTTPClientTransport(
          url,
          transportOptions(authProvider, serverConfig.headers)
        ),
      },
      {
        name: "SSE",
        ...
        if (isAuthError) {
          ...
          if (/registration|client_id/i.test(message)) {
            lastStatus = {
              ...
              error:
                "Server does not support dynamic client registration. Provide oauth.clientId in config.",
            }
          } else {
  ...
  private async startAuth(name: string) {
    ...
    const redirectUri =
      ...
      (oauthConfig?.callbackPort
        ? `http://127.0.0.1:${oauthConfig.callbackPort}/mcp/oauth/callback`
        : undefined)
    ...
    try {
      ...
      if (status.status === "needs_auth" && exchangedEntry?.tokens) {
        return {
          status: "failed",
          error:
            "OAuth callback completed and an access token was issued, but the server rejected it on reconnect. The token may have an incompatible audience/resource for this MCP endpoint.",
        } satisfies McpStatus
      }
      return status
    } catch (error) {
  ...
  private async storeClient(
    ...
    const collision = findToolKeyCollision(
      new Map(this.clients).set(name, { client, transport, config, tools })
    )
    if (collision) {
  ...
  private canAutoConnect(
    ...
    return (
      status === undefined || status.status === "disconnected" || status.status === "connecting"
    )
  }
  ...
  private async openAuthorizationUrl(
    url: string,
...
async function waitForConnectAttempt(
  attempt: Promise<McpStatus>,
