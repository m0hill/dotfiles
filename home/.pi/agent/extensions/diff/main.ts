...
function fileName(file: FileDiffMetadata) {
  return (file as any).name || "file"
}
function lineLabel(a: { scope?: string; side?: string; start?: number; end?: number }) {
  if (a.scope === "file") return "entire file"
  return [a.side, a.start ? `lines ${a.start}${a.end && a.end !== a.start ? `-${a.end}` : ""}` : ""]
    .filter(Boolean)
    .join(" ")
}
function rangeLines(r?: SelectedLineRange | null) {
  return {
    ...
    end: (r as any)?.end || (r as any)?.start,
    side: (r as any)?.side || (r as any)?.endSide || "additions",
  }
}
function stats() {
  let additions = 0,
  ...
  for (const f of files) {
    additions += (f as any).additionLines?.length ?? 0
    deletions += (f as any).deletionLines?.length ?? 0
  }
...
function quoteForRange(
  ...
  if (Array.isArray(rawLines) && rawLines.length) {
    ...
    return rawLines
      .slice(Math.max(0, start - 1), end || start)
      .map((line: string) => prefix + String(line).replace(/\n$/, ""))
      .join("\n")
  }
...
function renderShell() {
  ...
  document
    .querySelector("#leftToggle")!
    .addEventListener("click", () => document.body.classList.toggle("lc"))
  document
    .querySelector("#rightToggle")!
    .addEventListener("click", () => document.body.classList.toggle("rc"))
}
...
function renderDiffs() {
  ...
  files.forEach((file, i) => {
    ...
    outer
      ...
      .addEventListener("click", () => setFileSelection(file, "comment"))
    outer
      .querySelector("[data-ask]")!
      .addEventListener("click", () => setFileSelection(file, "ask"))
    ...
    instance.render({
      ...
      lineAnnotations: annotations
        .filter((a) => a.file === fileName(file) && a.scope === "lines" && a.start)
        .map((a) => ({
          side: (a.side || "additions") as any,
          lineNumber: a.start!,
...
function setLineSelection(file: FileDiffMetadata, range?: SelectedLineRange | null) {
  ...
  currentSelection = {
    ...
    side: rr.side,
    quote: quoteForRange(file, rr.side, rr.start, rr.end),
  }
...
function selectionDetailsHtml(selection: Selection): string {
  ...
  const lines = selection.start
    ? `${selection.start}${selection.end && selection.end !== selection.start ? `–${selection.end}` : ""}`
    : "unknown"
  return `<div class="selection-meta">
    <div class="selection-row"><span>File</span><code title="${esc(name)}">${esc(name)}</code></div>
...
function renderSelectionChooser(preferred?: "comment" | "ask") {
  ...
  document.querySelector("#chooseComment")!.addEventListener("click", () => {
    setMode("comment")
    renderCommentBox()
  })
  document.querySelector("#chooseAsk")!.addEventListener("click", () => {
    setMode("ask")
    renderAskBox()
  })
  if (preferred === "ask") {
...
function addAnnotation() {
  ...
  annotations.push({
    ...
    file: fileName(currentSelection.file),
...
function renderAnnotations() {
  ...
  root.querySelectorAll("[data-del]").forEach((btn) =>
    btn.addEventListener("click", () => {
      annotations.splice(Number((btn as HTMLElement).dataset.del), 1)
      renderAnnotations()
      renderDiffs()
    })
  )
...
async function sendFeedback() {
  ...
  if (!annotations.length && !globalComment)
    return alert("Add annotations or global feedback first.")
  await fetch("/api/feedback", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ id: review.id, annotations, globalComment }),
  })
...
async function askPi() {
  ...
  ;(document.querySelector("#askStatus") as HTMLElement).textContent =
    json.message || "Question sent to Pi. Check the terminal."
}
...
async function boot() {
  ...
  files = patches
    .flatMap((patch) => (Array.isArray(patch?.files) ? patch.files : [patch]))
    .filter(Boolean) as FileDiffMetadata[]
  renderShell()
  renderDiffs()
  renderAnnotations()
}
boot().catch((e) => {
  app.innerHTML = `<pre class="full-msg">${esc(String(e.message || e))}</pre>`
})
