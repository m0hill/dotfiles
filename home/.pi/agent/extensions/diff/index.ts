...
function writeSession(
  ...
  opts: { title: string; mode: ReviewMode; patch: string; commit?: string; base?: string }
): ReviewSession {
...
function feedbackText(session: ReviewSession, payload: FeedbackPayload): string {
  const annotations = (payload.annotations ?? []).filter(
    (a) => (a.comment ?? "").trim() || (a.quote ?? "").trim()
  )
  const lines = [`I reviewed the diff (${session.title}). Please address this feedback:`]
  annotations.forEach((a, i) => {
    const loc =
      a.scope === "file"
        ? `${a.file ?? "file"} entire file`
        : [
            a.file,
            a.side,
            a.start ? `lines ${a.start}${a.end && a.end !== a.start ? `-${a.end}` : ""}` : "",
          ]
            .filter(Boolean)
            .join(" ")
...
function askText(session: ReviewSession, payload: AskPayload): string {
  const loc =
    payload.scope === "file"
      ? `${payload.file ?? "file"} entire file`
      : [
          payload.file,
          payload.side,
          payload.start
            ? `lines ${payload.start}${payload.end && payload.end !== payload.start ? `-${payload.end}` : ""}`
            : "",
        ]
          .filter(Boolean)
          .join(" ")
...
function branchOptions(ctx: ExtensionCommandContext): string[] {
  ...
  function add(cmd: string[]) {
    try {
      for (const line of parseLog(runGit(ctx.cwd, cmd)))
        set.add(line.replace(/^refs\/remotes\//, ""))
    } catch {}
  }
...
async function chooseBase(ctx: ExtensionCommandContext): Promise<string | undefined> {
  ...
  if (!items.length) {
    ctx.ui.notify("No branches found to use as base.", "warning")
    return
  }
  ...
  pi.registerCommand("diff", {
    ...
    handler: async (_args, ctx) => {
      try {
        ...
        if (!patch.trim()) return ctx.ui.notify("No changes against HEAD.", "info")
        await openReview(
          pi,
          ctx,
  ...
  pi.registerCommand("diff-commit", {
    ...
    handler: async (_args, ctx) => {
      try {
        ...
        if (!patch.trim()) return ctx.ui.notify("Commit has no patch.", "info")
        await openReview(
          pi,
          ctx,
  ...
  pi.registerCommand("diff-base", {
    ...
    handler: async (_args, ctx) => {
      try {
        ...
        if (!patch.trim()) return ctx.ui.notify(`No changes against ${base}.`, "info")
        await openReview(
          pi,
          ctx,
