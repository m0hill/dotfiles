...
function getTuiSize(tui: { terminal?: { columns?: number; rows?: number } }): {
  width: number
  height: number
} {
  return {
...
function renderDashboardLines(
  ...
  const totalSecWidth = () =>
    secColWidths.slice(0, visibleSecMetrics.length).reduce((a, b) => a + b, 0)
  ...
  headerLine +=
    `${th.fg("muted", "status".padEnd(col.status))}` + `${th.fg("muted", "description")}`
  ...
  lines.push(
    truncateToWidth(`  ${th.fg("borderMuted", "─".repeat(Math.max(0, width - 4)))}`, width)
  )
...
export default function autoresearchExtension(pi: ExtensionAPI) {
  ...
  const autoresearchCompactionFor = (ctx: ExtensionContext, event: SessionBeforeCompactEvent) => {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const mergedASI =
        params.asi && Object.keys(params.asi).length > 0 ? (params.asi as ASI) : undefined
